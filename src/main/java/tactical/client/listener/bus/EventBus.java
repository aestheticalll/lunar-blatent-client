...
public class EventBus {
    ...
    private final Map<Class<?>, List<Subscriber>> eventListenerMap = new LinkedHashMap<>();
    ...
    public void subscribe(Object subscriber) {
        ...
        for (Field field : declaredFields) {
            if (Listener.class.isAssignableFrom(field.getType())) {
                ...
                int priority = DefaultEventPriority.DEFAULT;
                boolean ignoreCanceled = true;

                if (field.isAnnotationPresent(Subscribe.class)) {
                    Subscribe subscribe = field.getDeclaredAnnotation(Subscribe.class);
                    priority = subscribe.priority();
                    ignoreCanceled = subscribe.ignoreCanceled();
                }

                try {
                    Listener<?> listener = (Listener<?>) field.get(subscriber);
                    Class<?> eventClass = (Class<?>) ((ParameterizedType) field.getGenericType())
                            .getActualTypeArguments()[0];

                    List<Subscriber> subscriptions = eventListenerMap.computeIfAbsent(
                            eventClass, (x) -> new CopyOnWriteArrayList<>());

                    subscriptions.add(new Subscriber(subscriber, listener, priority, ignoreCanceled));
